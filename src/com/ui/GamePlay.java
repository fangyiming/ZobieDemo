...
public class GamePlay extends JPanel implements MouseListener, MouseMotionListener, Runnable{	
	...
	public void paintScreen(){
		...
		try{
			...
			Toolkit.getDefaultToolkit().sync();  // sync the display on some systems Synchronizes this toolkit's graphics state. Some window systems may do buffering of graphics events.
		    g.dispose();
		}catch(Exception e){
	...
	private void drawMoveImage(Graphics g) {
		if (selectedPlantType != PlantType.NONE) {			
			g.drawImage(selectedPlantType.getPlantImg(),mouseX + 2, mouseY + 2, null);  //在鼠标移动的位置绘制所选植物
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(selectedPlantType!=PlantType.NONE&&plantMap.inTheMap(clickMouseX, clickMouseY)){
